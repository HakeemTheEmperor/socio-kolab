...
import { ChevronsUpDown, LayoutGrid, LogOut, KeyRound, UserRound } from "lucide-react";
      ...
      <DropdownMenuContent align="start" className="w-56">
        ...
        {otherClubs.map((other) => (
          <DropdownMenuItem
            key={other.slug}
            render={
              <Link href={`/${other.slug}/dashboard`} onClick={onNavigate} />
            }
          >
            <ClubMark club={other} className="size-5 rounded text-[10px]" />
            <span className="truncate">{other.name}</span>
          </DropdownMenuItem>
        ))}
        <DropdownMenuSeparator />
        <DropdownMenuItem render={<Link href="/clubs" onClick={onNavigate} />}>
...
function UserMenu({
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="w-56">
        <DropdownMenuItem
          render={<Link href={`/${club.slug}/profile`} onClick={onNavigate} />}
        >
        ...
        <DropdownMenuItem
          render={
            <Link href={`/${club.slug}/profile#password`} onClick={onNavigate} />
          }
        >
...
export function Sidebar({
  ...
  return (
    <div className="flex h-full flex-col gap-2 bg-surface p-3">
      <ClubSwitcher club={club} otherClubs={otherClubs} onNavigate={onNavigate} />
      ...
      <UserMenu club={club} user={user} signOut={signOut} onNavigate={onNavigate} />
    </div>
  );
}
